...
def get_db():
    ...
    if _db is None:
        raise RuntimeError("MONGO_URI must include a database name")

    _db.users.create_index([("email", ASCENDING)], unique=True)
    _db.predictions.create_index([("createdAt", -1)])
    return _db
...
def validate_prediction_input(data):
    ...
    if not name or len(name) > 100:
        errors["name"] = "Student name is required and must be at most 100 characters"
    else:
        cleaned["name"] = name
...
def add_security_headers(response):
    ...
    response.headers["Cache-Control"] = "no-store" if request.path.startswith("/api/") else response.headers.get("Cache-Control", "public, max-age=3600")
    return response
...
def static_files(filename):
    # Serve only files that exist in the public directory; never expose project files.
    target = (PUBLIC_DIR / filename).resolve()
    if PUBLIC_DIR.resolve() not in target.parents:
...
def register():
    ...
    try:
        ...
        return jsonify({"message": "Account created successfully", "token": token, "user": {"name": name, "email": email}}), 201
    except Exception as exc:
        app.logger.exception("Registration failed")
        return jsonify({"error": "Unable to create account", "details": str(exc) if os.getenv("APP_DEBUG") == "true" else None}), 500
...
def login():
    ...
    try:
        ...
        token = make_token(user["_id"], user["email"])
        return jsonify({
            "message": "Login successful",
            "token": token,
            "user": {"name": user.get("name", "Student"), "email": user["email"]},
        })
    except Exception:
        app.logger.exception("Login failed")
        return jsonify({"error": "Unable to login right now"}), 500
...
def me():
    try:
        db = get_db()
        user = db.users.find_one({"email": request.user["email"]}, {"name": 1, "email": 1})
        if not user:
            return jsonify({"error": "User not found"}), 404
        return jsonify({"user": {"name": user.get("name", "Student"), "email": user["email"]}})
    except Exception:
        app.logger.exception("User lookup failed")
        return jsonify({"error": "Unable to load user"}), 500
...
def predict():
    ...
    try:
        ...
        features = pd.DataFrame([
            {
                ...
                "PreviousMarks": cleaned["previousMarks"],
                "InternalMarks": cleaned["internalMarks"],
        ...
        details = str(exc) if os.getenv("APP_DEBUG") == "true" else None
        return jsonify({"error": "Prediction failed", "details": details}), 500
...
def history():
    try:
        db = get_db()
        records = list(
            db.predictions.find({"userId": request.user["sub"]}, {"_id": 0})
            .sort("createdAt", -1)
            .limit(20)
        )
        for record in records:
            if isinstance(record.get("createdAt"), datetime):
...
if __name__ == "__main__":
    app.run(host="0.0.0.0", port=int(os.getenv("PORT", "5000")), debug=os.getenv("APP_DEBUG") == "true")
